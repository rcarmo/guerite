...
| `GUERITE_HEALTH_CHECK_TIMEOUT_SECONDS` | `120`   | Maximum seconds to wait for a recreated container to become `healthy` before triggering rollback.        |
| `GUERITE_PRUNE_TIMEOUT_SECONDS`        | `300`   | Docker API timeout (in seconds) used for image pruning. |
