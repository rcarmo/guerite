...
def test_main_loop_runs_single_iteration(monkeypatch, settings: Settings):
    ...
    monkeypatch.setattr(main_mod, "start_event_listener", lambda cfg, signal, client=None: None)
    monkeypatch.setattr(main_mod, "HttpServer", lambda *args, **kwargs: SimpleNamespace(start=lambda: None, stop=lambda: None))
